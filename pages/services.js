...
export default function ServicesPage() {
    return (
        <div className="container mx-auto px-4 py-16">
            {/* Hero Section */}
            <motion.div
                className="text-center mb-16 bg-gradient-to-r from-purple-600 to-indigo-700 text-white p-12 rounded-3xl shadow-xl"
                initial={{ opacity: 0, y: -50 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.7 }}
            >
                <h1 className="text-6xl font-extrabold leading-tight mb-4">
                    Nos Solutions de Livraison Globales
                </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-20">
                {serviceOfferings.map((service, index) => (
                    <motion.div
                        key={index}
                        className="bg-white p-10 rounded-2xl shadow-2xl border border-blue-100 flex flex-col items-center text-center transform hover:scale-105 transition-all duration-300 relative overflow-hidden"
                        initial={{ opacity: 0, y: 50 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.7, delay: 0.2 + index * 0.15 }}
                        whileHover={{ boxShadow: "0 20px 30px rgba(0,0,0,0.18)" }}
                    >
                        {/* Subtle background element for visual flair */}
                        <div className="absolute top-0 right-0 w-24 h-24 bg-blue-100 rounded-bl-full opacity-50 transform translate-x-1/2 -translate-y-1/2"></div>
                        <div className="absolute bottom-0 left-0 w-24 h-24 bg-blue-100 rounded-tr-full opacity-50 transform -translate-x-1/2 translate-y-1/2"></div>

                        <div className="p-5 rounded-full bg-blue-600 text-white mb-6 border-4 border-white z-10 shadow-lg">
                            <service.icon className="text-5xl" />
                        </div>
                        <h2 className="text-3xl font-bold text-gray-900 mb-4">{service.title}</h2>
                        <p className="text-gray-700 mb-8 flex-grow leading-relaxed">{service.description}</p>

                        <ul className="text-left text-gray-700 space-y-3 w-full mb-8">
                            {service.features.map((feature, idx) => (
                                <li key={idx} className="flex items-start">
                                    <FaRegCheckCircle className="text-green-500 mr-3 mt-1 flex-shrink-0" />
                                    <span>{feature}</span>
                                </li>
                            ))}
                        </ul>
                        ...
                        <Link href={service.link} passHref>
                            ...
                            >
                                ...
                                <FaClipboardCheck className="ml-3" />
                            </motion.button>
                        </Link>
                    </motion.div>
                ))}
            ...
            <motion.div
                className="text-center bg-blue-50 py-16 rounded-3xl shadow-inner mb-20 max-w-6xl mx-auto"
                initial={{ opacity: 0, y: 50 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 px-4">
                    ...
                    >
                        <FaGlobeAfrica className="text-blue-500 text-5xl mb-4" />
                        <h3 className="font-bold text-xl text-gray-900 mb-2">Expertise Locale & Internationale</h3>
                        <p className="text-gray-700 text-sm">Maîtrise parfaite des itinéraires Belgique-RDC.</p>
                    </motion.div>
                    {/* Value Prop 2 */}
                    <motion.div
                        className="flex flex-col items-center p-6 bg-white rounded-xl shadow-md border border-gray-100"
                    ...
                    >
                        <FaRegCheckCircle className="text-emerald-500 text-5xl mb-4" />
                        <h3 className="font-bold text-xl text-gray-900 mb-2">Fiabilité et Sécurité</h3>
                        <p className="text-gray-700 text-sm">Vos colis arrivent à destination en toute intégrité.</p>
                    </motion.div>
                    {/* Value Prop 3 */}
                    <motion.div
                        className="flex flex-col items-center p-6 bg-white rounded-xl shadow-md border border-gray-100"
                    ...
                    >
                        ...
                        <h3 className="font-bold text-xl text-gray-900 mb-2">Support Client Dévoué</h3>
                        <p className="text-gray-700 text-sm">Une équipe disponible pour toutes vos questions.</p>
                    </motion.div>
                    {/* Value Prop 4 */}
                    <motion.div
                        className="flex flex-col items-center p-6 bg-white rounded-xl shadow-md border border-gray-100"
                    ...
                    >
                        ...
                        <h3 className="font-bold text-xl text-gray-900 mb-2">Solutions Personnalisées</h3>
                        <p className="text-gray-700 text-sm">Des services adaptés à chaque besoin unique.</p>
                    </motion.div>
                </div>
            </motion.div>
            ...
            >
                <h2 className="text-5xl font-bold mb-6 leading-tight">
                    Prêt à Expédier avec Confiance ?
                </h2>
